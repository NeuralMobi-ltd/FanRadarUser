...
const normalizeOrder = (o) => {
  ...
  // Prefer pre-enriched items if present
...
const enrichOrderItems = async (ord) => {
...
const loadOrders = async () => {
  ...
  try {
    ...
    const normalized = await Promise.all(arr.map(async (o) => normalizeOrder(await enrichOrderItems(o))))
