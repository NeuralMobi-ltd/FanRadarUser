<template>
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 sm:p-4">
    <!-- Desktop-like mobile design with wider modal -->
          ...
          <div v-if="showTagInput || tags.length" class="pl-16">
            <div class="flex flex-wrap gap-3 mb-4" v-if="Array.isArray(tags) && tags.length">
              <span
                ...
                :key="idx"
                class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200"
              >
                #{{ tag }}
...
async function submit() {
  if (!postContent.value.trim() && postMedia.length === 0) return
