import API_CONFIG from '@/config/api'
import http from '@/services/http'

export const FollowsService = {
	async follow(userId){
		// Ensure valid integer id
		const uid = Number(userId)
		if (!Number.isInteger(uid) || uid <= 0) throw new Error('Invalid user id')
		// POST follow endpoint; expect 201 or success message
		const { data } = await http.post(API_CONFIG.users.follow(uid))
		return data
	},
	async unfollow(userId){
		const uid = Number(userId)
		if (!Number.isInteger(uid) || uid <= 0) throw new Error('Invalid user id')
		// DELETE unfollow endpoint; backend may return 200 with success or 4xx if not following
		const { data } = await http.delete(API_CONFIG.users.unfollow(uid))
		return data
	},
	async toggle(userId) {
		const uid = Number(userId)
		if (!Number.isInteger(uid) || uid <= 0) throw new Error('Invalid user id')
		const { data } = await http.post(API_CONFIG.users.follow(uid))
		// Expected shape per docs: { following:true|false, follower_count, following_count }
		return data
	},
	async followers(userId, params = {}) {
		const uid = Number(userId)
		if (!Number.isInteger(uid) || uid <= 0) throw new Error('Invalid user id')
		const { data } = await http.get(API_CONFIG.users.followers(uid), { params })
		return data
	},
	async following(userId, params = {}) {
		const uid = Number(userId)
		if (!Number.isInteger(uid) || uid <= 0) throw new Error('Invalid user id')
		const { data } = await http.get(API_CONFIG.users.following(uid), { params })
		return data
	},
	async userPosts(userId, params = {}) {
		const uid = Number(userId)
		if (!Number.isInteger(uid) || uid <= 0) throw new Error('Invalid user id')
		const { data } = await http.get(API_CONFIG.users.posts(uid), { params })
		return data
	}
}

export default FollowsService
